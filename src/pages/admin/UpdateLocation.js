...
function UpdateLocation() {
  ...
  useEffect(() => {
    // Fetch existing data for the location
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/v1/locations/${id}`);
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:8000/api/v1/locations/${id}`, data, {
        headers: {
