...
const Booking = ({ tour, avgRating }) => {
    ...
    if (id === "roomQuantity") {
      if (value < 1 || value > booking.adult) {
        setErrors((prev) => ({
          ...prev,
          roomQuantity:
            "Number of rooms must be at least 1 and cannot exceed the number of adults.",
        }));
      } else {
    ...
    if (id === "extraBed") {
      if (value < 0 || value > booking.roomQuantity) {
        setErrors((prev) => ({
          ...prev,
          extraBed:
            "Number of extra beds must be between 0 and the number of rooms.",
        }));
      } else {
  ...
  const handleSelectChange = (e) => {
    ...
    if (name === "hotelId") {
      ...
      setBooking((prev) => ({
        ...
        bedPrice: selectedHotel ? selectedHotel.bedPrice : 0,
      }));
  ...
  const serviceFee = 10;
  const totalAmount =
    (Number(booking.hotelPrice) * Number(booking.roomQuantity) +
      Number(booking.extraBed) * Number(booking.bedPrice)) *
      itineraryDays +
    (Number(price) * Number(booking.adult) +
      0.7 * Number(price) * Number(booking.children)) +
    serviceFee;
    ...
    <div className="booking">
      ...
      <div className="booking__form">
        ...
        <Form className="booking__info-form" onSubmit={handleClick}>
          ...
          <p className="text-danger">
            {errors.adult && <small>{errors.adult}</small>}
          </p>
          <p className="text-danger">
            {errors.children && <small>{errors.children}</small>}
          </p>
          <p className="text-danger">
            {errors.baby && <small>{errors.baby}</small>}
          </p>
          ...
          <p className="text-danger">
            {errors.roomQuantity && <small>{errors.roomQuantity}</small>}
          </p>
          <p className="text-danger">
            {errors.extraBed && <small>{errors.extraBed}</small>}
          </p>
        ...
        <ListGroup>
          <ListGroupItem className="border-0 px-0">
            <h5 className="d-flex align-items-center gap-1">
              ${price} <i className="ri-close-line"></i> {booking.adult} adults,{" "}
              {booking.children} children(sale 30%)
            </h5>
            <span>
              $
              {(
          ...
          <ListGroupItem className="border-0 px-0">
            <h5>
              Hotel Cost: {booking.hotelPrice}(room price) x{" "}
              {booking.roomQuantity} x {itineraryDays} day
            </h5>
            <span>
              $
              {(
                Number(booking.hotelPrice) *
                Number(booking.roomQuantity) *
                itineraryDays
              ).toFixed(2)}
            </span>
          </ListGroupItem>
          <ListGroupItem className="border-0 px-0">
            <h5>
              Extra Bed Cost: {booking.bedPrice}(bedPrice) x {booking.extraBed}{" "}
              x {itineraryDays} day
            </h5>
            <span>
              $
              {(
                Number(booking.extraBed) *
                Number(booking.bedPrice) *
                itineraryDays
              ).toFixed(2)}
            </span>
          </ListGroupItem>
